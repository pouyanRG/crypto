...
export function TableCell({ children = null, className = undefined, header = false, scope = undefined, numeric = false }) { const Tag = header ? "th" : "td"; return <Tag scope={header ? scope ?? "col" : undefined} className={clsx("whitespace-nowrap px-4 py-2 text-left", numeric && "font-tabular text-right", header && "text-xs font-medium uppercase tracking-wide", className)}>{children}</Tag>; }
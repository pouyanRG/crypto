...
export default function CoinStats({ coin }) {
  ...
  return <section><h2 className="mb-3 text-base font-semibold text-[var(--color-text-primary)]">Key statistics</h2><dl className="grid grid-cols-2 gap-3 lg:grid-cols-4">{STAT_LABELS.map(([key, label]) => <Card as="div" variant="compact" key={key}><dt className="text-xs text-[var(--color-text-muted)]">{label}</dt><dd className="mt-2 font-tabular text-sm font-medium text-[var(--color-text-primary)]">{formatValue(key, marketData[key])}</dd></Card>)}</dl></section>;
}